...
@click.option(
    ...
    default=stacs.constants.CACHE_DIRECTORY,
)
@click.argument("path")
def main(
    debug: bool,
    ...
    try:
        pack = stacs.model.pack.from_file(rule_pack)
    ...
    if ignore_list:
        ...
        try:
            ignored = stacs.model.ignore_list.from_file(ignore_list)
        except stacs.exceptions.STACSException as err:
    ...
    try:
        targets = stacs.loader.filepath.finder(path, cache_directory)
    except stacs.exceptions.STACSException as err:
    ...
    findings = []
    for scanner in stacs.scanner.__all__:
        try:
            findings.extend(getattr(stacs.scanner, scanner).run(targets, pack))
        except stacs.exceptions.InvalidFormatException as err:
